  ...
  (p) => ({
    ...
    detail:
      p.lucidRatio >= 15
        ...
        : p.lucidRatio >= 8
          ? `Normal lucid frekansı — platform stabil`
          : `Düşük lucid oran — derin uyku döngüsü hâkim`,
  }),

  (p) => ({
    category: 'emotion',
    color: p.nightmareRatio > 35 ? '#FF4A5E' : '#FF8C00',
    icon: '◉',
...
export function LiveSystemProvider({ children }: { children: ReactNode }) {
  ...
  const { data: liveStream } = useQuery({
    ...
    queryFn: () => fetchLiveStream(24, 30),
    refetchInterval: 8_000,
    staleTime: 6_000,
  });
  const { data: aiSignals } = useQuery({
    queryKey: ['ai-signals'],
    queryFn: () => fetchAISignals(10),
    refetchInterval: 30_000,
    staleTime: 25_000,
  });
  ...
  useEffect(() => {
    ...
    const sigEvents = aiSignals.slice(0, 3).map((s) => {
      ...
      return {
        ...
        icon: cfg.icon,
        color:
          s.severity === 'critical' ? '#FF3060' : s.severity === 'warning' ? '#FF8C00' : cfg.color,
        message: s.message,
  ...
  useEffect(() => {
    ...
    const real = activity.slice(0, 5).map((a) => {
      const cat: LiveEventCategory =
        a.type === 'dream_created' ? 'dream' : a.type === 'login' ? 'system' : 'ai';
      const cfg = CAT_CONFIG[cat];
      return {
        id: uid(),
        category: cat,
        ...
        color: cfg.color,
        message:
          a.type === 'dream_created' ? `@${a.username} yeni rüya` : `@${a.username} sisteme giriş`,
        detail: a.detail,
  ...
  useEffect(() => {
    ...
    const snapshot: PlatformSnapshot = {
      symbols:
        collective?.sharedSymbols
          .filter((s) => s.symbol && s.symbol.length < 25)
          .map((s) => s.symbol) ?? FALLBACK_SYMBOLS,
  ...
  const systemPulse = Math.min(
    100,
    Math.max(0, Math.round((health?.communityHealthScore ?? 50) + lucidBonus - reportPenalty)),
  );
