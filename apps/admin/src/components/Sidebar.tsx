...
const NAV: NavSection[] = [
  ...
  },
  {
    ...
    color: '#CC80FF',
    icon: '◈',
    items: [{ to: '/ai-center', icon: '◈', label: 'AI Core', badge: 'Beta' }],
  },
  {
    title: 'CONTROL',
    color: '#FF8C00',
...
function NavSectionGroup({ section }: { section: NavSection }) {
  const { pathname } = useLocation();
  const isActivSection = section.items.some(
    (item) =>
      pathname === item.to || (item.to !== '/dashboard' && pathname.startsWith(item.to + '/')),
  );
  ...
  return (
    <div className="mb-0.5">
      ...
      >
        <span
          className="text-[8px] font-bold uppercase tracking-[0.2em] flex-1"
        ...
        <div className="px-3 py-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
          <div className="flex items-center gap-2.5">
            ...
            >
              ...
              >
                <path
                  d="M10 12l4-4-4-4M14 8H6M6 3H3a1 1 0 00-1 1v8a1 1 0 001 1h3"
                  strokeLinecap="round"
                />
              </svg>
            </button>
