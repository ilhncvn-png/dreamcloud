...
import {
  fetchOverview, fetchCommunityHealth, fetchConsciousnessMap,
  fetchCollectiveConsciousness, fetchEmotionMap, fetchOperationalAlerts,
} from '../api/admin.api';
import { useLiveSystem } from '../contexts/LiveSystemContext';

/* ── AI panel section ────────────────────────────────────────────────── */

function AISection({ title, color, children }: {
  title: string; color: string; children: React.ReactNode;
}) {
  return (
    <div className="p-4 rounded-xl" style={{ background: `${color}06`, border: `1px solid ${color}14` }}>
      <p className="text-[8px] font-mono font-black tracking-[0.2em] mb-2" style={{ color: `${color}AA` }}>{title}</p>
      {children}
    </div>
  );
}
...
export default function DreamAICore() {
  ...
  const ch  = useQuery({ queryKey: ['com-health'],            queryFn: fetchCommunityHealth,         staleTime: 50_000 });
  const cm  = useQuery({ queryKey: ['consciousness-map'],     queryFn: fetchConsciousnessMap,        staleTime: 50_000 });
  const cc  = useQuery({ queryKey: ['collective-consciousness'], queryFn: fetchCollectiveConsciousness, staleTime: 50_000 });
  ...
  useEffect(() => { setPulse(p => p + 1); }, [events[0]?.id]);
  ...
  const emotionSummary = emotion
    ? `${emotion.dominantEmotion.toUpperCase()} dominant frekansta %${
        emotion.topEmotions[0]?.pct ?? 0
      }. Duygusal hava: ${
        emotion.dominantType === 'positive' ? 'güneşli ve açık' :
        emotion.dominantType === 'negative' ? 'fırtınalı ve derin' : 'bulutlu ve geçişsel'
  ...
  const collectiveSummary = collective
    ? `Kolektif bilinç ${collective.coherenceLevel} seviyesinde. ` +
      ...
      `Dominant duygu: ${collective.collectiveEmotion.toUpperCase()}.`
    : 'Kolektif alan taranıyor…';

  const riskSummary = critAlerts > 0
    ...
    <>
      ...
      >
        ...
        <svg viewBox="0 0 24 24" fill="none" style={{ width: 22, height: 22 }}>
          {open ? (
            <path d="M6 18L18 6M6 6l12 12" stroke="rgba(255,255,255,0.9)" strokeWidth="2" strokeLinecap="round" />
          ) : (
            <>
              ...
              {[0, 60, 120, 180, 240, 300].map(deg => {
                ...
                return (
                  <circle key={deg}
                    cx={12 + 8 * Math.cos(rad)} cy={12 + 8 * Math.sin(rad)}
                    r="1" fill="rgba(123,111,255,0.6)" />
                );
              })}
            </>
          )}
      ...
      {open && (
        <div ref={panelRef}
          ...
          <div style={{
            ...
            background: 'linear-gradient(90deg, transparent, rgba(204,128,255,0.7), rgba(0,207,255,0.4), transparent)',
          }} />
          ...
          <div className="flex items-center justify-between px-5 py-4"
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="w-6 h-6 rounded-full flex items-center justify-center"
                  style={{ background: 'rgba(123,111,255,0.2)', border: '1px solid rgba(123,111,255,0.4)' }}>
              ...
              <div>
                <p className="text-[11px] font-black tracking-widest font-mono" style={{ color: '#CC80FF' }}>
                  DREAM AI CORE
                </p>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[8px] font-mono" style={{ color: '#38D68A' }}>PULSE {systemPulse}</span>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            {recentSignals.length > 0 && (
              <div style={{ borderBottom: '1px solid rgba(123,111,255,0.08)', paddingBottom: 12, marginBottom: 12 }}>
                <p className="text-[8px] font-mono font-black tracking-[0.2em] mb-2" style={{ color: 'rgba(123,111,255,0.4)' }}>
                  CANLI SİNYAL
                </p>
                <div className="space-y-1.5">
                  {recentSignals.map((e, i) => (
                    <div key={e.id} className="flex items-start gap-2"
                      ...
                      <span className="text-[9px] font-mono mt-0.5 shrink-0" style={{ color: e.color }}>{e.icon}</span>
                      <div>
                        <p className="text-[10px] font-mono" style={{ color: 'rgba(232,232,255,0.8)' }}>{e.message}</p>
                        {e.detail && <p className="text-[8px] font-mono" style={{ color: 'rgba(232,232,255,0.35)' }}>{e.detail}</p>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <AISection title="TOPLULUK RUHALI" color="#38D68A">
              <p className="text-[11px] leading-relaxed" style={{ color: 'rgba(232,232,255,0.75)' }}>
                {moodSummary}
              </p>
            </AISection>
            ...
            <AISection title={critAlerts > 0 ? `RİSK ALARMI · ${critAlerts} KRİTİK` : 'RİSK DEĞERLENDİRMESİ'} color={critAlerts > 0 ? '#FF4A5E' : '#5A5A84'}>
              <p className="text-[11px] leading-relaxed" style={{ color: 'rgba(232,232,255,0.75)' }}>
                {riskSummary}
              </p>
            </AISection>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div key={label} className="p-2 rounded-lg text-center"
                  ...
                  <p className="text-[8px] font-mono mb-0.5" style={{ color: `${color}80` }}>{label}</p>
                  <p className="font-mono font-black text-sm" style={{ color }}>{typeof value === 'number' ? value.toLocaleString() : value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="px-5 py-3 flex items-center justify-between"
            ...
            <div className="flex items-center gap-1">
              <span className="text-[8px] font-mono animate-ai-cursor" style={{ color: 'rgba(204,128,255,0.6)' }}>▋</span>
            </div>
          </div>
        </div>
      )}
